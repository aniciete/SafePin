...
serve(async (req)=>{
  ...
  try {
    ...
    if (!filePath || !reportId) {
      return new Response(JSON.stringify({
        error: 'Missing filePath or reportId'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: report, error: reportError } = await supabaseAdmin.from('reports').select('user_id').eq('id', reportId).single();
    ...
    const { data: userRole } = await supabaseAdmin.from('users').select('role').eq('id', user.id).single();
    const isOwner = report.user_id === user.id;
    const isAdmin = userRole?.role === 'admin';
    if (!isOwner && !isAdmin) {
      return new Response(JSON.stringify({
        error: 'User is not authorized to delete this image'
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    const { error: deleteError } = await supabaseAdmin.storage.from('report-images').remove([
      filePath
    ]);
    if (deleteError) {
