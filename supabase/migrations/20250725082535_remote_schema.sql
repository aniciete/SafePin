...
CREATE OR REPLACE FUNCTION "public"."assign_jurisdiction_on_report_insert"() RETURNS "trigger"
...
DECLARE
  ...
  report_lat FLOAT;
BEGIN
  -- Extract longitude and latitude from the location JSONB
  report_lon := (NEW.location->>'lng')::float;
  report_lat := (NEW.location->>'lat')::float;

  -- Only proceed if we have a location
  IF report_lon IS NOT NULL AND report_lat IS NOT NULL THEN
    -- Call our lookup function to get the jurisdiction
    SELECT get_jurisdiction_for_location(report_lon, report_lat) INTO found_psgc;

    -- If a jurisdiction was found, update the new report record
    IF found_psgc IS NOT NULL THEN
      NEW.jurisdiction := found_psgc;
    END IF;
  END IF;
...
BEGIN
  SELECT psgc_code
  INTO psgc_result
  FROM public.jurisdiction_boundaries
  WHERE 
...
ALTER FUNCTION "public"."get_jurisdiction_for_location"("lat" double precision, "lng" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_report_stats"() RETURNS TABLE("status" "public"."report_status", "count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION "public"."get_report_stats"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_top_jurisdictions_by_pending_reports"("limit_count" integer) RETURNS TABLE("jurisdiction" "text", "jurisdiction_name" "text", "pending_reports_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        r.jurisdiction,
        r.jurisdiction AS jurisdiction_name, -- Placeholder, can be joined with a jurisdictions table later
        count(r.id) as pending_reports_count
    FROM
        public.reports r
    WHERE
        r.status = 'pending_verification' AND r.jurisdiction IS NOT NULL
    GROUP BY
        r.jurisdiction
    ORDER BY
...
ALTER FUNCTION "public"."get_top_jurisdictions_by_pending_reports"("limit_count" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_user_jurisdiction"() RETURNS "text"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_user_jurisdiction"() OWNER TO "postgres";
...
ALTER TABLE "public"."jurisdiction_boundaries" OWNER TO "postgres";


ALTER TABLE "public"."jurisdiction_boundaries" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."jurisdiction_boundaries_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS "public"."users" (
    ...
    CONSTRAINT "admin_cannot_have_jurisdiction" CHECK ((NOT (("role" = 'admin'::"public"."user_role") AND ("jurisdiction" IS NOT NULL))))
);
...
ALTER TABLE ONLY "public"."jurisdiction_boundaries"
    ADD CONSTRAINT "jurisdiction_boundaries_psgc_code_key" UNIQUE ("psgc_code");
...
ALTER TABLE ONLY "public"."reports"
    ADD CONSTRAINT "reports_tracking_code_key" UNIQUE ("tracking_code");
...
CREATE INDEX "idx_reports_is_flagged" ON "public"."reports" USING "btree" ("is_flagged") WHERE ("is_flagged" = true);



CREATE INDEX "idx_reports_jurisdiction" ON "public"."reports" USING "btree" ("jurisdiction");



CREATE INDEX "idx_reports_tracking_code" ON "public"."reports" USING "btree" ("tracking_code");



CREATE INDEX "idx_users_jurisdiction" ON "public"."users" USING "btree" ("jurisdiction");
...
CREATE OR REPLACE TRIGGER "trigger_assign_jurisdiction" BEFORE INSERT ON "public"."reports" FOR EACH ROW EXECUTE FUNCTION "public"."assign_jurisdiction_on_report_insert"();
...
CREATE POLICY "Allow authorities to view reports in their jurisdiction" ON "public"."reports" FOR SELECT USING (((( SELECT "users"."role"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"())) = 'authority'::"public"."user_role") AND ("jurisdiction" = "public"."get_user_jurisdiction"())));
...
GRANT ALL ON FUNCTION "public"."assign_jurisdiction_on_report_insert"() TO "anon";
GRANT ALL ON FUNCTION "public"."assign_jurisdiction_on_report_insert"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."assign_jurisdiction_on_report_insert"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_report_stats"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_report_stats"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_report_stats"() TO "service_role";



GRANT ALL ON FUNCTION "public"."get_top_jurisdictions_by_pending_reports"("limit_count" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_top_jurisdictions_by_pending_reports"("limit_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_top_jurisdictions_by_pending_reports"("limit_count" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_user_jurisdiction"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_jurisdiction"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_jurisdiction"() TO "service_role";
...
GRANT ALL ON TABLE "public"."jurisdiction_boundaries" TO "anon";
GRANT ALL ON TABLE "public"."jurisdiction_boundaries" TO "authenticated";
GRANT ALL ON TABLE "public"."jurisdiction_boundaries" TO "service_role";



GRANT ALL ON SEQUENCE "public"."jurisdiction_boundaries_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."jurisdiction_boundaries_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."jurisdiction_boundaries_id_seq" TO "service_role";
